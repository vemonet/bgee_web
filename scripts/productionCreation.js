...
const main = async () => {
  try {
    ...
    if (config.archive) {
      prod = false;
      console.log('\x1b[31m[ERROR] %s\x1b[0m', 'config.json is set as an archive');
    }
    if (scss.indexOf('$archive: false;') === -1) {
      prod = false;
      console.log('\x1b[31m[ERROR] %s\x1b[0m', 'styles.scss is set as an archive');
    }
    if (html.indexOf(noIndexSource) !== -1) {
      prod = false;
      console.log('\x1b[31m[ERROR] %s\x1b[0m', 'noindex meta tag are put in public/index.html');
    }
    if (pkg.homepage) {
      prod = false;
      console.log('\x1b[31m[ERROR] %s\x1b[0m', 'Unwanted "homepage" key is defined in package.json');
    }
