    ...
    case TOP_ANAT_FLOW.LAUNCHING_JOB:
      return (
        <Bulma.Notification color="warning" className="mt-5">
          ...
          <p>
            Processing job, waiting for a job id. Please stand by until the job is started, or until results are
            retrieved, if they already exist on our server (in that case, we won&apos;t send you a notification email,
            if you provided your address)
          </p>
        </Bulma.Notification>
      );
    case TOP_ANAT_FLOW.GOT_JOB:
      return (
        <Bulma.Notification color="warning" className="mt-5">
          ...
          <p>
            After bookmarking this page, it is safe to close this window. Your analysis is being run on our server, and
            the results will appear as soon as available. Please note that the results can be slow to compute, typically
            from 5 to 30 minutes, depending on the amount of data to process. It is not necessary to refresh this page,
        ...
        <Bulma.Notification color="info" className="my-5">
          <p ref={refGotResult}>
            {`TopAnat request successful. Found ${nbRecords} ${isPlural(
              ...
              'analyse',
              nbAnalysisSuccess
            )} with results, over ${nbAnalysis} ${isPlural('analyse', nbAnalysis)} launched.`}
          </p>
        </Bulma.Notification>
      );
    default:
