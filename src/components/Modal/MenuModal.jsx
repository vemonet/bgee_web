  ...
  return (
    <>
      <button
        type="button"
        aria-haspopup="true"
        aria-controls="bgee-complete-menu"
        aria-expanded="false"
      ...
      <ModalCard active={active} toggle={toggle} />
    </>
  );
};
...
const ModalCard = ({ active, toggle }) => (
  <div id="bgee-complete-menu" className={`modal menu-modal ${active ? 'is-active' : ''}`}>
    ...
    <div className="modal-card menu-modal-card ">
      ...
      >
        <Bulma.Image alt="Bgee logo" className="logo" src={assets.bgeeLogo} height={40} width={99} />
      </Link>
            ...
            <ul className="menu-list">
              {children.map(({ key: keyChild, type, title: pageTitle, ...childProps }) => {
                ...
                switch (type) {
                  ...
                  case 'fullinternal':
                    link = (
                      <a className="navbar-item custom" href={childProps.path}>
                        {pageTitle}
                      </a>
                    );
                    break;
